...
exports.cors= cors(); //When we call cors, it will return to us a middleware function configured to set a cors header of Access Control Allow Origin on our response object with a wildcard as its value (which means it will allow cors for all origins). This is for Simple Requests.
exports.corsWithOptions= cors(corsOptionsDelegate); //This cors function will return to us a middleware function and will check to see if the incoming request belongs to the whitelisted origins (localhost:3000 or localhost:3443). If it does, it will send us back the cors response header of Access Control Allow Origin (this time with the whitelisted origin as the value). If it doesn't, no corse header. This is for Preflighted Requests.